...
def plot_weighted_euclidean_distance(
    ...
    original_ticks = [round(x_v, 3) for x_v in original_ticks]
    ax2.set_xticks(original_ticks)
    for label in ax2.get_xticklabels():
        label.set_rotation(90)
    ax2.tick_params(axis='x', direction='in', pad=-45, labelsize=13)  
        
    ax2.legend(loc=3, prop={'size': 14}) # lower
    ax2.set_yscale("log")
    ax2.set_ylabel("The factorial Euclidean Distance $\mathbf{H}_{j,i}$", color="black", fontsize=15)
    ax2.set_xlabel("The weighted Euclidean Distance $\mathbf{H}_{j}$", color="black", fontsize=16)
    ...
    fig.tight_layout()
    plt.savefig(DIRS_DATA_SS_FIG + r'\Distance_compliance_relationship_w.png', dpi=400)
...
def reasonSolutionSpaceWeighted(
    ...
    if calc_distance:
        # # weighted * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
        # find from the sorted results in 'valid_via_weighted_distance.csv'
        ...
        # 1512                  1559                    0.690
        # 669                   1657                    0.739
        res_plot_nr = [0, 1282, 1522, 1718, 1512, 669] # designnumber
        res_plot_idx = [0, 381-1, 482-1, 1079-1, 1559-1, 1657-1] # choose and add
        # # weighted * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
